...
You would want to use the mean statistic $\bar{x}$
...
They are just that - estimates. They're only estimations of the actual thing,
and they can be manipulated by things like outliers and bad sampling.
...
I wouldn't be surprised with 3, but I would be with 40.
...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-jt-galla/manhattan.csv")
```
...
- Parameter of interest: Rent cost
- Sample: Randomly sampled 1-bedroom apartments in Manhattan
- Sample size: 20
...
This means that we are 95% confident the true mean (avg) of 1-bedroom Manhattan 
apartments is between 2299 and 2977.
...
Interpretation: This means that we are 95% confident the true median (middle) of 
1-bedroom Manhattan apartments is between 2162.5 and 2875.
...
- Population: Mice
- Parameter of interest: Dead or Not Dead after Specific Dose
- Sample: Mice given Doses
...
This means that we are 95% confident the true proportion of mice who died while
not on the treatment regardless of dose is between 0.690 and 0.797.
...
```{r practice_2_ci}
ci_mice_died_prx2 <- get_ci(boot_prop_prx2, level = .90)
ci_mice_died_prx2
```
...
visualise(boot_prop_prx2) +
  shade_ci(ci_mice_died_prx2)
```


Give an interpretation of your result.

This means that we are 90% confident the true proportion of mice who died while
on the treatment regardless of dose is between 0.349 and 0.448.
